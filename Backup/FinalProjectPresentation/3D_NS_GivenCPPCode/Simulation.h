...
class Simulation {
  ...
  public:
    ...
    virtual void plotVTK(int timeStep){
      ...
      FieldIterator<FlowField> vtkIterator( _flowField, _parameters,vtkStencil, 1, 0 );
  ...
  protected:
    ...
    virtual void setTimeStep(){
      ...
      FLOAT factor = 1.0/(_parameters.geometry.dx * _parameters.geometry.dx) +
      ...
      if (_parameters.geometry.dim == 3) {
        factor += 1.0/(_parameters.geometry.dz * _parameters.geometry.dz);
        _parameters.timestep.dt = _parameters.geometry.dz / _maxUStencil.getMaxValues()[2];
      } else {
        _parameters.timestep.dt = _parameters.geometry.dx / _maxUStencil.getMaxValues()[0];
      }
